...
Aonde \textit{k} é a constante de Boltzman, \textit{T} é a temperatura do ambiente em Kelvin e \textit{B} é a banda em hertz.
...
Cada modo depende de diferentes parâmetros de entrada. No modo ponto-a-ponto, informações detalhadas do terreno para o caminho entre o transmissor e receptor estão disponíveis, as variáveis adicionais de entrada utilizados pelo algoritmo nesse modo são:
...
Os parâmetros citados anteriormente são utilizados para computar a perda da transmissão. O resultado é um valor de atenuação calculado através do somatório de duas atenuações: A primeira é utilizando o Free Space citado anteriormente enquanto a segunda é uma atenuação de referência, que pode ser calculada de diferentes maneiras, dependendo da distância entre os dispositivos.
...
Sistemas REST tipicamente utilizam o protocolo HTTP com os mesmos métodos (GET,POST,PUT E DELETE) utilizado pelos browsers para carregar páginas e enviar dados a servidores.
...
\subsection{Classes}
A linguagem utilizada para a implemetanção do servidor foi Java, uma linguagem orientada a objetos onde todos os arquivos são classes. O padrão de desenvolvimento adotado por Machado~\cite{tccmarcelo} foi o de Fábricas (\textit{Factories}).
